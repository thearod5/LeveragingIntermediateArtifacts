...
from api.constants.processing import Data, METRIC_COLNAME, PERCENT_COLNAME, VALUE_COLNAME, CORRELATION_COLNAME, \
    P_VALUE_COLNAME, N_SIG_FIGS, TRANSITIVE_TRACE_TYPE_COLNAME, DATASET_COLNAME
...
def create_correlation_matrix(data: Data):
    ...
    """
    assert all([col in data for col in [METRIC_COLNAME, VALUE_COLNAME, PERCENT_COLNAME]]), data.columns
    result = pd.DataFrame()
...
def create_gain_correlation_table(dataset_name: str,
                                  sampling_type: str,
                                  correlation_df: Data):
    experiment_label = ExperimentTraceType.ALL.value if sampling_type == SamplingExperiment.TRACES.value \
    ...
    gain_for_trace_df = gain_df[trace_mask].reset_index(drop=True).drop(TRANSITIVE_TRACE_TYPE_COLNAME, axis=1)
